...
class DynamicCardStacker(CardStacker):
    ...
    def _get_spacing_free_room(self) -> float:
        return max(1.0, self.height - IMAGE_HEIGHT)

    @property
    def _spacing(self):
        return min(self._max_spacing, self._get_spacing_free_room() / max(len(self._cards), 1))
...
class DynamicStackingGrid(StackingGrid):
    ...
    def request_space(self, card_stacker: CardStacker, x: int, y: int) -> None:
        for i in range(card_stacker.y_index, self._stacker_map.column_height):
            max_requested = max(
                [stacker.requested_size[1] for stacker in self._stacker_map.row_at(i)] + [self._min_row_height]
            )
