        ...
        for scene_type in SceneType:
            ...
            if "aligner_options" not in current_value[scene_type.value]:
                current_value[scene_type.value]["aligner_options"] = ALIGNER_TYPE_MAP[
                    current_value[scene_type.value]["aligner_type"]
                ].schema.default
...
DOUBLECLICK_MATCH_ON_CARDBOARDS = BooleanSetting(
    "doubleclick_match_on_cardboards", "Doubleclick matches on cardboards", True
)
...
CONFIRM_CLOSING_MODIFIED_FILE = BooleanSetting(
    "confirm_closing_modified_file", "Confirm closing modified / unsaved files", True
)
AUTO_LOGIN = BooleanSetting("auto_login", "Auto login", False)
DEFAULT_FOCUS_TRAP_SUB_PRINTING = BooleanSetting(
    "default_focus_trap_sub_printing", "Default focus trap sub-printing", False
)
...
REMOTE_IMAGE_URL = StringSetting(
    "remote_image_url", "Remote images url", "https://prohunterdogkeeper.dk", requires_restart=True
)
ALLOW_LOCAL_IMAGE_FALLBACK = BooleanSetting(
    "allow_local_image_fallback", "Allow local fallback", True, requires_restart=True
)
...
EXTERNAL_STORE_DB = BooleanSetting(
    "external_store_db", "Use external database for application storage", False, requires_restart=True
)
EXTERNAL_DATABASE_NAME = StringSetting(
    "external_database_name", "External database name", "embargo", requires_restart=True
)
