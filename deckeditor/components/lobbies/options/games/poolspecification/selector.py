...
from deckeditor.components.lobbies.options.games.poolspecification.boosters.allcardsbooster import (
    AllCardsBoosterSpecificationSelector,
)
from deckeditor.components.lobbies.options.games.poolspecification.boosters.chaosbooster import (
    ChaosBoosterSpecificationSelector,
)
from deckeditor.components.lobbies.options.games.poolspecification.boosters.cubebooster import (
    CubeBoosterSpecificationSelector,
)
from deckeditor.components.lobbies.options.games.poolspecification.boosters.expansionbooster import (
    ExpansionBoosterSpecificationSelector,
)
...
class PoolSpecificationSelector(QtWidgets.QWidget):
    def __init__(self, lobby_view: LobbyViewInterface):
        ...
        layout.addWidget(
            self._add_booster_specification_button,
            1,
            1,
            1,
            1,
    ...
    def _on_booster_specification_value_change(self, option: str, value: t.Any) -> None:
        ...
        if (
            ...
            and current_options[0]["type"] == CubeBoosterSpecification.__name__
            and option == "release"
        ):
    ...
    def _on_booster_specification_index_change(
        ...
        self._booster_specifications_selector.update_content(
            self._lobby_view.lobby.game_options["pool_specification"][self._current_specification_index], self._enabled
        )
